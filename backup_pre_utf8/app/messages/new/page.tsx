...
const schema = z.object({
  recipient_email: z.string().email('Неверный email'),
  content_text: z.string().min(1, 'Введите текст послания').max(5000, 'Слишком длинное послание'),
...
export default function NewMessagePage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setMessage({ text: 'Послание успешно сохранено!', type: 'success' });
  ...
  return (
    <div className="bg-white text-gray-900 font-sans min-h-screen flex flex-col">
      <section className="container mx-auto px-4 py-16 md:py-24 text-center">
        <h1 className="text-4xl md:text-5xl lg:text-6xl font-extrabold tracking-tight leading-tight mb-4">Новое послание</h1>
        <p className="text-lg md:text-xl text-gray-600 max-w-2xl mx-auto mb-12">Напишите важное сообщение, которое будет отправлено в нужный момент.</p>
      </section>
      ...
      <section className="container mx-auto px-4 pb-16 max-w-2xl">
        <form onSubmit={handleSubmit} className="space-y-6" noValidate>
          ...
          <div>
            <label htmlFor="recipient_email" className="block text-sm font-medium text-gray-700">Email получателя</label>
          ...
          <div>
            <label htmlFor="content_text" className="block text-sm font-medium text-gray-700">Текст послания</label>
          ...
          <div>
            <label htmlFor="mode" className="block text-sm font-medium text-gray-700">Режим отправки</label>
            <select id="mode" name="mode" value={formData.mode} onChange={handleChange}
              className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-gray-500 focus:ring-gray-500 sm:text-sm">
              <option value="dms">Немедленно (как только возможно)</option>
          ...
          {formData.mode === 'scheduled' && (
            <div>
              <label htmlFor="delivery_at" className="block text-sm font-medium text-gray-700">Дата и время доставки</label>
              <input id="delivery_at" name="delivery_at" type="datetime-local" required value={formData.delivery_at}
                onChange={handleChange}
                className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-gray-500 focus:ring-gray-500 sm:text-sm" />
              <p className="mt-1 text-xs text-gray-500">Будет сохранено в UTC, локаль: Europe/Oslo</p>
            </div>
          )}
          ...
          <button type="submit" disabled={isLoading}
            ...
            {isLoading ? 'Сохранение...' : 'Отправить послание'}
          </button>
        </form>
      </section>
    </div>
